...
```{r vote and pid3 by state, echo=TRUE, eval=TRUE}
...
```{r overall mail percentage, echo=TRUE, eval=TRUE}
...
```{r overall plot, echo=TRUE, eval=TRUE, fig.align="center"}
...
Combine dataset.
```{r combine all years, echo=TRUE, eval=TRUE}
...
```{r overall plot by party, echo=TRUE, eval=TRUE, fig.align="center"}
...
```{r us states, echo=TRUE, eval=TRUE}
...
Find geoid for each us states. Possible reference link could be *[kjhealy/fips-codes](https://github.com/kjhealy/fips-codes)*
```{r geoid, echo=TRUE, eval=TRUE}
...
```{r combine with abbreviations, echo=TRUE, eval=TRUE}
usfullabb<-us.states %>%
...
```{r combine bystate with geoid, echo=TRUE, eval=TRUE}
bystategeoid<-left_join(bystatejoin, geoid, by="state.abb")
bystategeoid
```

Obtain shape files of US states using the `tigris` package.
```{r shapefiles, echo=TRUE, eval=TRUE}
stateshape<-states(cb=TRUE)
```

Read in covid cases data. *Data source is [United States CDC](https://data.cdc.gov/Case-Surveillance/United-States-COVID-19-Cases-and-Deaths-by-State-o/9mfq-cb36)*
```{r covid data, echo=TRUE, eval=TRUE}
...
Keep only data during the voting period. The election day is November 3, and the earliest voting time is 46 days before the election day. Reference from *[Early Voting Calendar](https://www.vote.org/early-voting-calendar/)*. The end of our observation date is 7 days after the election day.
...
To make the number of cases comparable, they need to be weighted by state population. *Data Source is [United States Census Bureau](https://www.census.gov/programs-surveys/popest/technical-documentation/research/evaluation-estimates.html)*

```{r population, echo=TRUE, eval=TRUE}
...
```{r preprocess populations, echo=TRUE, eval=TRUE}
...
```{r combine covid and population dataset, echo=TRUE, eval=TRUE}
...
```{r weighted data, echo=TRUE, eval=TRUE}
covidw<-covidw %>%
  arrange(submission_date) %>%
...
```{r total number, echo=TRUE, eval=TRUE}
totalcovid<-covidw %>%
  group_by(state.abb) %>%
  filter(row_number()==1) %>%
  ungroup() %>%
...
```{r all data, echo=TRUE, eval=TRUE}
...
```{r merge state data with covid, echo=TRUE, eval=TRUE}
...
```{r 1st map, echo=TRUE, eval=TRUE, fig.width=9, fig.height=7}
...
pal=colorNumeric(palette="Purples", domain=allbystate$mail) 
leafletmap1<-leaflet() %>%
  addTiles() %>%
...
```{r 2nd map, echo=TRUE, eval=TRUE, fig.width=9, fig.height=7}
content<-paste("State:", allbystate$STATE, "<br/>",
               "Total number of cases:", allbystate$statetotalduringvoting, "<br/>",
               "Weighted total number of cases:", allbystate$statetotalw, "<br/>",
              "Percentage of mail-in voting:", allbystate$mail, "<br/>",
...
```{r plot relationship, echo=TRUE, eval=TRUE, fig.align="center"}
